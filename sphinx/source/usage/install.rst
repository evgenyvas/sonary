...
Sonary is an audio server, which scans your library and allows to view collection, listen it via external local player and convert to mp3.
...
* SERVE_FRONTEND - Allows to serve frontend via standart template (templates/index.html). If disabled, only /api and /ws endpoints will be available
* ROOT_PATHS — Paths to the directories where your music is stored
* PATH_MAP - Path transformation rules, which used for play via foobar2000
* DATABASE_DSN - Path to SQLite database
* WORKER_COUNT - Number of maximum parallel running jobs
* FOOBAR_PATH - Path to foobar2000 executable
* FOOBAR_API - Use beefweb plugin for API requests. If disabled, foobar2000 CLI will be used
* FOOBAR_API_URL - Address for beefweb API requests
* CACHE_DIR - Path for image thumbnails
