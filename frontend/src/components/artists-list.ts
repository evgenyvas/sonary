...
export class ArtistsList extends SonaryLitElement {
    ...
    relatedArtistId: number | null = null
    ...
    private get queryKey(): string {
        return getArtistsKey(this.query)
    }

    connectedCallback() {
        super.connectedCallback()
        if (this.storeState.artists.currentKey !== this.queryKey) {
            this._loadItems()
        }
    }
    ...
    _loadItems() {
        ...
        this.store.dispatch(fetchArtists(this.query, this.limit, this._page)).then(() => {
